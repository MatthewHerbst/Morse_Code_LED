/*
 ...
 TODO:
  ...
  5) DONE: Add a speaker for playing a tone.  For some reason the tone(,, delay ) did not work, so I had to do delay post starting the tone
  ...
  8) Now that we are operating off of strings directly, it is now possible to send "canned messages"; thus I abstracted out the guts of the translation to a subroutine.
*/
...
// Morse code tone 
// Found a reference on Widipedia that 600 -> 1000 HZ 
// https://en.wikipedia.org/wiki/Morse_code#Link_budget_issues
#define MIN_TONE    31     // right out of arduino documentation
#define MORSE_TONE  1200   // split the difference
#define MAX_TONE    2048   // bit arbitrary but at upper range of most cheap speakers
#define SPEAKER_PIN  9     // change this as needed based on wiring
...
/*
 * MORSE_SEND()
 *  takes a text string, and then sends legal morse characters.
 *  Currently that means blinking LED and sending a tone on the speaker (tone can optionally be set as 2nd parameter.
 */
void morse_send( char *morse_message, int optional_tone ) 
// *morse_message;  0 terminated string that is to be sent
// optional_tone;  if <> 0, then tone played will be @ designated frequency.  will do limit checks, and default tone if outside limits
{
  ...
  // limit check the tone
  if ( optional_tone == 0 || optional_tone < MIN_TONE || optional_tone > MAX_TONE ) {  // first check is duplicate as long as MIN_TONE > 0
      tone_freq = MORSE_TONE;
  } else {
      tone_freq = optional_tone;
  }
...
void loop() {
  ...
  for ( x = 400; x <= 1200; x += 10 ) {
    morse_send( Input_string, x );
  }
}
